...
# =============================================
# タイトルから数字・記号・スペースを除去して文字だけ取り出す
# =============================================
def extract_title_text(title: str) -> str:
    """「1.工 事 概 要」→「工事概要」のように数字・記号・スペースを除去"""
    # 先頭の番号部分（半角・全角数字＋ドット）を除去
    t = re.sub(r"^[\d０-９]+[\.．]\s*", "", title)
    # 残ったスペース（半角・全角）を除去
    t = t.replace("　", "").replace(" ", "")
    return t.strip()
...
# カラーパレット（くすみポップ系）
# base_color: タブの基本色
# light_color: ハイライト（立体感の上部）
# dark_color: シャドウ（立体感の下部・影）
# =============================================
PALETTE = [
...
def draw_3d_tab(page: fitz.Page, rect: fitz.Rect, palette: dict, is_active: bool):
    ...
    if is_active:
        # アクティブ：上部ハイライト帯 ＋ 下部ベース色 ＋ 右側に影
        h = y1 - y0
        # 上1/3をライト色
        top_rect = fitz.Rect(x0, y0, x1, y0 + h * 0.35)
        s = page.new_shape()
        s.draw_rect(top_rect)
        s.finish(color=light, fill=light)
        s.commit()
        # 下2/3をベース色
        bot_rect = fitz.Rect(x0, y0 + h * 0.35, x1, y1)
        s = page.new_shape()
        s.draw_rect(bot_rect)
        s.finish(color=base, fill=base)
        s.commit()
        # 左アクセントライン（濃い色）
        accent = fitz.Rect(x0, y0, x0 + 4, y1)
        s = page.new_shape()
        s.draw_rect(accent)
        s.finish(color=dark, fill=dark)
        s.commit()
        # 下部シャドウライン
        ...
        s = page.new_shape()
        s.draw_rect(top_rect)
        s.finish(color=faded_light, fill=faded_light)
        s.commit()
        bot_rect = fitz.Rect(x0, y0 + h * 0.30, x1, y1)
        s = page.new_shape()
        s.draw_rect(bot_rect)
        s.finish(color=faded_base, fill=faded_base)
        s.commit()
        # 薄い下部ライン
        ...
        s.finish(color=faded_dark, fill=faded_dark)
        s.commit()
...
def apply_smart_tabs(doc: fitz.Document, chapters: list, page_mapping: list) -> fitz.Document:
    ...
    for i in range(doc.page_count):
        ...
        tab_h = min(38, available_height / max(num_chapters, 1))
        y_offset = 10

        for c_idx, chapter in enumerate(chapters):
            is_active = (c_idx == current_chapter_idx)
            palette = PALETTE[c_idx % len(PALETTE)]
            ...
            if is_active:
                ...
                # タイトル文字のみ（数字・記号・スペース除去）
                title_text = extract_title_text(chapter["title"])
                if len(title_text) > 6:
                    title_text = title_text[:5] + "…"
                title_rect = fitz.Rect(8, y_offset + tab_h * 0.38, MARGIN - 2, y_offset + tab_h - 3)
                page.insert_textbox(title_rect, title_text, fontsize=6.5,
                                    fontname="helv", color=(1, 1, 1), align=fitz.TEXT_ALIGN_LEFT)
            else:
                # 非アクティブ：番号のみ中央
